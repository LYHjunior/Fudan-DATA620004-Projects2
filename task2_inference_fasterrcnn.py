...
def main(cfg):
    ...
    if VOC:
        ...
        # in this situation, we draw object detection results for figure NOT in VOC2007
        detection_threshold = 0.7
        image_dir = [p for p in Path('mid-homework/imgs/task2_output/input').rglob("*.jpg")]
        for img_path in image_dir:
            ...
            if len(outputs[0]['boxes']) != 0:
                ...
                draw_box(orig_image,draw_boxes,pred_classes,scores,CLASSES,COLORS,label=True)
                cv2.waitKey(1)
                cv2.imwrite(f"mid-homework/imgs/task2_output/output/faster_rcnn/{img_path.name}", orig_image)
